...
hands = pygame.display.set_mode((EDGE,EDGE))
...
while running:
    ...
    for (radius, theta, color, stroke) in (
        ...
        (SECOND_R, second_theta, BLACK, SECOND_STROKE),
    ):
        line_at_angle(hands,CENTER,radius,theta,color,stroke)
        pygame.transform.scale(hands,(WIDTH,HEIGHT))
        #line_at_angle(screen, CENTER, radius, theta, color, stroke)
        hands.blit(hands,(WIDTH,HEIGHT))
